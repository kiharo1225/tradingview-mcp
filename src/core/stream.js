...
async function pollLoop(fetcher, { interval = 500, dedupe = true, label = 'stream' } = {}) {
  ...
  process.stderr.write(`   Streams from your locally running TradingView chart via Desktop or browser mode.\n`);
  process.stderr.write(`   Does not connect to TradingView servers directly. Requires a local CDP target on port 9222.\n`);
